...
ITypeInstruction::ITypeInstruction(byte opcode, byte rd, byte func3, byte rs1, bytes imm) {
  if (imm.size() != 2) {
    throw new AbstractInstructionException("Failed to set imm, length not 2 bytes [%d]", imm.size());
  }

  ulong valOfImm = getBytesToULong(imm);
  if (valOfImm > MAX_IMM_SIZE) {
    throw new AbstractInstructionException("Failed to set imm, size greater than allowed [max: %d, acc: %d]", MAX_IMM_SIZE, valOfImm);
  }
  ...
  if (rs1 > R_MAX) {
    throw new AbstractInstructionException("Failed to set rs1, greater than 31 [%d]", rs1);
  }
  this->rs1 = rs1;
  this->imm = imm;
...
void ITypeInstruction::decode(bytes instruction) {
  if (instruction.size() != INSTRUCTION_SIZE) {
    throw new AbstractInstructionException("Failed to decode instruction, not 4 bytes");
  }
