...
ITypeInstruction::ITypeInstruction(byte opcode, byte rd, byte func3, byte rs1, bytes imm) {
  if (imm.size() != 2) {
    throw new InstructionException("Failed to set imm, length not 2 bytes [%d]", imm.size());
  }

  ulong valOfImm = getBytesToULong(imm);
  if (valOfImm > MAX_IMM_SIZE) {
    throw new InstructionException("Failed to set imm, size greater than allowed [max: %d, acc: %d]", MAX_IMM_SIZE, valOfImm);
  }
...
bytes ITypeInstruction::getImm(ushort low, ushort high) {
  ...
  throw new InstructionException("Failed to get imm, wrong position [low: %d, high: %d]", low, high);
}