...
JTypeInstruction::JTypeInstruction(byte opcode, byte rd, byte imm8, byte imm1, bytes imm10, byte imm31) {
  ...
  if (imm8 > IMM8_MAX) {
    throw new AbstractInstructionException("Failed to set imm8, greater than 255 [%d]", imm8);
  }
  this->imm[0] = imm8;
  ...
  if (imm10.size() != IMM10_SIZE || imm10Size > IMM10_MAX) {
    throw new AbstractInstructionException("Failed to set imm10, greater than 1023 [%d]", imm10Size);
  }
  ...
  if (imm31 > IMM1_MAX) {
    throw new AbstractInstructionException("Failed to set imm10, greater than 1 [%d]", imm31);
  }
  this->imm[2] |= imm31 << 3;
